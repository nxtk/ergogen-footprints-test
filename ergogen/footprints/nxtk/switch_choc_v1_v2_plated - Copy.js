...
module.exports = {
  ...
  (footprint "ceoloide:switch_choc_v1_v2"
    ...
    (pad "" ${p.plated ? 'thru_hole' : 'np_thru_hole'} circle (at 5.5 0 ${p.r}) (size ${p.plated ? p.choc_v1_stabilizers_diameter + 0.3 : p.choc_v1_stabilizers_diameter} ${p.plated ? p.choc_v1_stabilizers_diameter + 0.3 : p.choc_v1_stabilizers_diameter}) (drill ${p.choc_v1_stabilizers_diameter}) (layers "*.Cu" "*.Mask"))
    (pad "" ${p.plated ? 'thru_hole' : 'np_thru_hole'} circle (at -5.5 0 ${p.r}) (size ${p.plated ? p.choc_v1_stabilizers_diameter + 0.3 : p.choc_v1_stabilizers_diameter} ${p.plated ? p.choc_v1_stabilizers_diameter + 0.3 : p.choc_v1_stabilizers_diameter}) (drill ${p.choc_v1_stabilizers_diameter}) (layers "*.Cu" "*.Mask"))
    `
    ...
    ${'' /* Middle Hole */}
    (pad ${(p.plated && !p.reversible) ? 1 : '""'} ${p.plated ? 'thru_hole' : 'np_thru_hole'} circle (at 0 -5.95 ${p.r}) (size ${p.plated ? '3.3 3.3' : '3 3'}) (drill 3) (layers "*.Cu" "*.Mask") ${(p.plated && !p.reversible) ? p.to.str : ''})
    `
    ...
    const hotswap_back_pad_plated = `
    (pad "1" smd roundrect (at -3.245 -5.95 ${p.r}) (size 2.65 2.15) (layers "B.Cu" "B.Paste" "B.Mask") (roundrect_rratio 0.1) ${p.from.str})
    (pad "1" smd roundrect (at -2.648 -5.95 ${p.r}) (size 3.8 2.15) (layers "B.Cu") (roundrect_rratio 0.1) ${p.to.str})
    (pad "" smd roundrect (at -3.248 -5.95 ${p.r}) (size 2.6 2.15) (layers "B.Paste" "B.Mask") (roundrect_rratio 0.1) ${p.to.str}
    (pad "2" smd roundrect (at ${7.6475 - (2.6 - p.outer_pad_width_back) / 2} -3.75 ${p.r}) (size ${p.outer_pad_width_back + 1.2} 2.15) (layers "B.Cu") (roundrect_rratio 0.1) ${p.from.str})
    (pad "2" smd roundrect (at ${8.2475 - (2.6 - p.outer_pad_width_back) / 2} -3.75 ${p.r}) (size ${p.outer_pad_width_back} 2.15) (layers "B.Paste" "B.Mask") (roundrect_rratio ${(2.15 / p.outer_pad_width_back) <= 1 ? 0.1 : 0.1 * (2.15 / p.outer_pad_width_back)}) ${p.from.str}))
    `

    const hotswap_back_pad_plated_reversible = ``

    const hotswap_front_pad_plated = `
    (pad "2" smd roundrect (at 3.245 -5.95 ${p.r}) (size 2.65 2.15) (layers "F.Cu" "F.Paste" "F.Mask") (roundrect_rratio 0.1) ${p.to.str})
    (pad "1" smd roundrect (at 2.648 -5.95 ${p.r}) (size 3.8 2.15) (layers "F.Cu") (roundrect_rratio 0.1) ${p.to.str})
    (pad "" smd roundrect (at 3.248 -5.95 ${p.r}) (size 2.6 2.15) (layers "F.Paste" "F.Mask") (roundrect_rratio 0.1) ${p.to.str}) 
    (pad "2" smd roundrect (at ${-7.6475 - (2.6 - p.outer_pad_width_back) / 2} -3.75 ${p.r}) (size ${p.outer_pad_width_back + 1.2} 2.15) (layers "F.Cu") (roundrect_rratio 0.1) ${p.from.str})
    (pad "2" smd roundrect (at ${-8.2475 - (2.6 - p.outer_pad_width_back) / 2} -3.75 ${p.r}) (size ${p.outer_pad_width_back} 2.15) (layers "F.Paste" "F.Mask") (roundrect_rratio ${(2.15 / p.outer_pad_width_back) <= 1 ? 0.1 : 0.1 * (2.15 / p.outer_pad_width_back)}) ${p.from.str})
    `

    const hotswap_front_pad_plated_reversible = ``
    ...
    ${!p.plated ?
        p.reversible ?
            hotswap_back_pad_cutoff
          : hotswap_back_pad_full
        : ''}

    ${'' /* Right Pad (not cut off) */}
    ${!p.plated ? `(pad "1" smd rect (at ${8.2475 - (2.6 - p.outer_pad_width_back) / 2} -3.75 ${p.r}) (size ${p.outer_pad_width_back} 2.6) (layers "B.Cu" "B.Paste" "B.Mask") ${p.to.str})` : ''}
    
    ${'' /* Side Hole */}
    (pad ${p.plated ? `"2" thru_hole` : `"" np_thru_hole`} circle (at 5 -3.75 ${195 + p.r}) (size ${p.plated ? '3.3 3.3' : '3 3'}) (drill 3) (layers "*.Cu" "*.Mask") ${p.from.str})
    `
    ...
    ${!p.plated ?
        p.reversible ?
          hotswap_front_pad_cutoff
        : hotswap_front_pad_full
      : ''}

    ${'' /* Left Pad (not cut off) */}
    ${p.plated ? `` : `(pad "2" smd rect (at ${-8.2475 + (2.6 - p.outer_pad_width_front) / 2} -3.75 ${p.r}) (size ${p.outer_pad_width_front} 2.6) (layers "F.Cu" "F.Paste" "F.Mask") ${p.to.str})`}
